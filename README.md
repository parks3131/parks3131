...
Software Engineer with 2+ years building backend systems, AI-powered platforms, and production observability infrastructure. At SUNY Research Foundation, developed backend services using Python and FastAPI, managed data across Amazon RDS (PostgreSQL) and DynamoDB, and maintained CI/CD pipelines as part of a 5-person Agile team, shipping a nationally distributed digital exhibit platform now serving museum partners across the United States. At Acarin, owns the full reliability stack for an enterprise AI HR platform — real-time telemetry pipelines built with Docker, InfluxDB, and Grafana, stress-tested to 20,000 concurrent users, and a Playwright/Cucumber BDD automation suite deployed in CI/CD that eliminated manual regression entirely. Built Interstellar, a developer intelligence platform that detects drift between what engineers intend and what ships, powered by FastAPI, LLM-based code analysis, and AI evals triggered on every commit.
